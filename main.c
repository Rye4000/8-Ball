...
static char *ResponsePool[20] = {"It is certain.",
                                 "It is decidedly so.",
                                 "Without a doubt.",
                                 "Yes - definitely.",
                                 "You may rely on it.",
...
static char ResponseFavorString[3] = {'+', '-', 'X'};

static char *ResponseFavorColor[3] = {ANSI_COLOR_GREEN, ANSI_COLOR_RED,
                                      ANSI_COLOR_YELLOW};

enum ResponseFavor { F_GOOD = 0, F_NEUTRAL, F_BAD };
...
/*
 *Generat a random integer between min and max.
 */
...
void Print_MagicResponse(global_t *g) {
  int randomIndex = Random_Get(0, 19);

  /*calculate Favor*/
  int favor = (randomIndex <= 9)                       ? F_GOOD
              : (randomIndex > 9 && randomIndex <= 14) ? F_NEUTRAL
                                                       : F_BAD;

  /*print response*/
  fprintf(stdout, "%s[%c]%s %s\n", ResponseFavorColor[favor],
          ResponseFavorString[favor], ANSI_COLOR_RESET,
...
void Update_Main(global_t *this) {
  while (TRUE) {
    ...
    if (this->responseString[0] != '\0') {
      ...
      if (!strcmp(this->responseString, "y") ||
          !strcmp(this->responseString, "yes")) {
        continue;
      } else if (!strcmp(this->responseString, "n") ||
                 !strcmp(this->responseString, "no")) {
        break;
      } else if (!strcmp(this->responseString, "exit()")) {
        goto ExecuteExit;
      } else {
        fprintf(stdout, "I'm sorry, I didn't understand that.\n");
